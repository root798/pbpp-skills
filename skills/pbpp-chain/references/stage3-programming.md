...
| N6 | Deterministic fiscal validator: totals, per-program deltas, delta sum, constraint status. An overrun is INFEASIBLE — the chain never rebalances another line to force a pass (`repair_applied` stays false). | equality/ceiling as declared |
